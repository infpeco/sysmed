...
if($Estado=='Nuevo')
{
      ...
      $sql = "INSERT INTO `empleado`(`id`, `Nombre`, `Apellido`, `Direccion`, `Identificacion`, `TipoIdentificacion`, `Sexo`, `TipoSangre`, `SeguroSocial`, `Area`, `Cargo`, `Escolaridad`, `Departamento`, `Especialidad`, `FechaEntrada`, `FechaNacimiento`, `Correo`, `Experiencia`,`Tipo`,`Estado`) VALUES (NULL,'$Nombre','$Apellido','$Direccion','$Identificacion',$TipoIdentificacion,'$Sexo','$TipoSangre',$SeguroSocial,'$Area','$Cargo','$Escolaridad','$Departamento',NULL,'$FechaEntrada','$FechaNaciemiento','$Correo',$Experiencia,'$Tipo','$EstadoReal');";
}
else
{     
      $Id=$data['id'];
      $sql = "UPDATE `empleado` SET `Nombre`= '$Nombre',`Apellido`='$Apellido', `Direccion`='$Direccion', `Identificacion`='$Identificacion', `TipoIdentificacion`=$TipoIdentificacion, `Sexo`='$Sexo', `TipoSangre`='$TipoSangre', `SeguroSocial`='$SeguroSocial',  `Experiencia`=$Experiencia, `Departamento`='$Departamento', `Cargo`='$Cargo', `Area`='$Area', `FechaEntrada`='$FechaEntrada', `FechaNacimiento`='$FechaNaciemiento', `Correo`='$Correo', `Estado`='$Estado', `Escolaridad`='$Escolaridad' WHERE `empleado`.`id` = $Id; ";
}

...
if($Estado=='Nuevo')
{
      ...
      $sql = "INSERT INTO `empleado`(`id`, `Nombre`, `Apellido`, `Direccion`, `Identificacion`, `TipoIdentificacion`, `Sexo`, `TipoSangre`, `SeguroSocial`, `Area`, `Cargo`, `Escolaridad`, `Departamento`, `Especialidad`, `FechaEntrada`, `FechaNacimiento`, `Correo`, `Experiencia`,`Tipo`,`Estado`) VALUES (NULL,'$Nombre','$Apellido','$Direccion','$Identificacion',$TipoIdentificacion,'$Sexo','$TipoSangre',$SeguroSocial,NULL,NULL,'$Escolaridad',NULL,'$Especialidad','$FechaEntrada','$FechaNaciemiento','$Correo',$Experiencia,'$Tipo','$EstadoReal');";
}
else
{
      $Id=$data['id'];
      $sql = "UPDATE `empleado` SET `Nombre`= '$Nombre',`Apellido`='$Apellido', `Direccion`='$Direccion', `Identificacion`='$Identificacion', `TipoIdentificacion`=$TipoIdentificacion, `Sexo`='$Sexo', `TipoSangre`='$TipoSangre', `SeguroSocial`='$SeguroSocial',  `Experiencia`=$Experiencia, `Especialidad`='$Especialidad', `FechaEntrada`='$FechaEntrada', `FechaNacimiento`='$FechaNaciemiento', `Correo`='$Correo', `Estado`='$Estado', `Escolaridad`='$Escolaridad' WHERE `empleado`.`id` = $Id; ";
}
